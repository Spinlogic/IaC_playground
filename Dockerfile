...
RUN apt updgrade -y; apt install sudo; echo "$username ALL=(ALL) NOPASSWD:ALL" >> /etc/sudoers; chmod -R go= /home/$username/.ssh; chown -R $username:$username /home/$username/.ssh; chmod a-x /home/$username/.ssh/authorized_keys; usermod -a -G sudo $username;
